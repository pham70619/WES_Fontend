...
const saveRecord = (_record) => {
  if (Mode.value === 'Add') {
    ...
    emit('updateData', { action: 'add', record: _record, component: 'WCS' })
  } else if (Mode.value === 'Edit') {
    console.log(`Item ${_record.id} has been edited!`)
    emit('updateData', { action: 'edit', record: _record, component: 'WCS' })
  }
  isEditModalOpen.value = false
}
...
const deleteRecord = (_record) => {
  ModalHandler.confirm({
    ...
    content: '是否確定刪除資料?',
    onOk: async () => {
      console.log(`WCS/${_record.type} data with ID: ${_record.id} has been deleted!`)
      emit('updateData', { action: 'delete', record: _record, component: 'WCS' })
    },
    onCancel: () => {
...
<template>
  <div class="m-3 pb-3 gap-3 border-bottom d-flex align-items-center position-relative">
    ...
    <p class="m-0 position-absolute start-50">設定下發WCS資料的方式</p>
  </div>
